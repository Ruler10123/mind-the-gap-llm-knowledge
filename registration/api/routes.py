...
from models.schemas import QRAuthRequest, AuthResponse, UserResponse, TicketInfo, StatusInfo
...
def set_services(db: DatabaseService, face: FaceRecognitionService, qr: QRService):
    """Set service instances (called from main.py on startup)."""
...
@router.post("/auth/face", response_model=AuthResponse)
async def authenticate_face(
    ...
    try:
        ...
        if similarity_score < settings.SIMILARITY_THRESHOLD:
            return AuthResponse(
                status="failure",
                message=f"Face similarity too low (score: {similarity_score:.3f}, threshold: {settings.SIMILARITY_THRESHOLD})",
...
@router.post("/auth/qr", response_model=AuthResponse)
